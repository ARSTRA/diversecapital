        ...
        <div className="grid grid-cols-1 gap-4">
          {isLoading ? (
            ...
            <div className="text-center py-24 bg-white rounded-[32px] border-2 border-dashed border-slate-200">
              ...
              <p className="text-muted-foreground font-medium">No messages found in your inbox.</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center gap-6">
                      <div className={cn(
                        ...
                        message.isRead ? "bg-slate-50 text-slate-400" : "bg-secondary/10 text-secondary"
                      )}>
                        {message.isRead ? <Mail className="w-6 h-6" /> : <Mail className="w-6 h-6 animate-bounce-slow" />}
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-3">
                          <span className={cn("text-sm font-bold", message.isRead ? "text-primary/70" : "text-primary")}>
                            {message.name}
                          </span>
                          {!message.isRead && (
                            <span className="bg-secondary text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">New</span>
                          )}
                        </div>
                        <h4 className={cn("text-base font-bold", message.isRead ? "text-primary/60" : "text-primary")}>
                          {message.subject}
                        </h4>
                    ...
                    <div className="text-right flex items-center gap-6">
                      <div className="flex flex-col items-end">
                        ...
                        <span className="text-[10px] text-muted-foreground mt-1">{message.email}</span>
                      </div>
                ...
                <DialogContent className="max-w-2xl rounded-[32px]">
                  <DialogHeader>
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        <DialogTitle className="text-2xl font-bold text-primary">{message.name}</DialogTitle>
                        <p className="text-sm text-secondary font-medium">{message.email}</p>
                      </div>
                    </div>
                  </DialogHeader>
                  <div className="space-y-6 pt-4 border-t border-slate-100">
                    <div>
                      <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Subject</span>
                      <h4 className="text-lg font-bold text-primary mt-1">{message.subject}</h4>
                    </div>
                    <div>
